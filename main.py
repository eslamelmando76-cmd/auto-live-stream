...
async def broadcast():
    print("--- [System] YouTube Live Starting ---")
    if not YOUTUBE_KEY or YOUTUBE_KEY == "None":
        print("--- [Error] YOUTUBE_KEY (via TWITCH_KEY secret) is missing! Check Secrets. ---")
        return
    ...
    while True:
        ...
        video_files = []
        # البحث عن فيديو تقني مناسب من Pexels
        ...
        # فلتر توحيد الأبعاد لـ 240p لضمان استقرار البث ومنع الـ Mismatch
        filter_complex = (
        ...
        # أمر FFmpeg المعدل لزيادة الاستقرار ودعم البث المستمر
        cmd = [
            ...
            "-flvflags", "no_duration_filesize", # ضروري جداً لاستمرار البث دون انقطاع
            TWITCH_URL
        ]

        subprocess.run(cmd)
        # فترة انتظار بسيطة قبل توليد القصة التالية لضمان عدم حدوث ضغط
